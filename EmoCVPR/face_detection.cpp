...
//************************************
// Method:   非极大值抑制
// FullName:NonMaximumSuppression    
// Returns: 处理后的人脸结构向量
// Parameter: 输入人脸结构，阈值，阈值方式
// Timer:2017.5.2
//************************************
...
// Parameter: 模型文件夹
// Timer:2017.5.2
void MTCNN::LoadModle(const string& proto_model_dir){
#ifdef CPU_ONLY
...
void MTCNN::ClassifyFace_MulImage(const std::vector<FaceInfo>& regressed_rects, cv::Mat &sample_single,
	...
	// ONet points_offset != NULL
	const boost::shared_ptr<Blob<float> > points_offset = (netName == 'o' ? net->blob_by_name(pointsLayerName) : nullptr);
